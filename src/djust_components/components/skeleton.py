...
class Skeleton(Component):
    ...
    def _render_custom(self) -> str:
        ...
        if self.skeleton_type == "card":
            inner = "".join(f'<div class="skeleton-line"></div>' for _ in range(max(1, self.lines)))
            cls = "skeleton-card"
            if self.custom_class:
                cls += f" {html.escape(self.custom_class)}"
            return (
        ...
        if self.skeleton_type == "table":
            rows = "".join(f'<div class="skeleton-line"></div>' for _ in range(max(1, self.lines)))
            cls = "skeleton-table"
            if self.custom_class:
                cls += f" {html.escape(self.custom_class)}"
            return (
        ...
        # Default: text lines
        line_html = "".join(f'<div class="skeleton-line"></div>' for _ in range(max(1, self.lines)))
        cls = "skeleton-text"
        if self.custom_class:
            cls += f" {html.escape(self.custom_class)}"
