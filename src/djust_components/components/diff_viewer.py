...
class DiffViewer(Component):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            old=old, new=new, mode=mode, title_old=title_old,
            title_new=title_new, show_line_numbers=show_line_numbers,
            custom_class=custom_class, **kwargs,
        )
    ...
    def _render_split(self, class_str, ops):
        ...
        for tag, old_line, new_line in ops:
            if tag == "equal":
                old_num += 1
                new_num += 1
                num_o = f'<span class="dj-diff__num">{old_num}</span>' if self.show_line_numbers else ""
                num_n = f'<span class="dj-diff__num">{new_num}</span>' if self.show_line_numbers else ""
                old_rows.append(
                ...
                old_num += 1
                num_html = f'<span class="dj-diff__num">{old_num}</span>' if self.show_line_numbers else ""
                old_rows.append(
                ...
                new_num += 1
                num_html = f'<span class="dj-diff__num">{new_num}</span>' if self.show_line_numbers else ""
    ...
    def _render_unified(self, class_str, ops):
        ...
        return (
            f'<div class="{class_str}">'
            f'<div class="dj-diff__unified">{"".join(rows)}</div></div>'
        )
