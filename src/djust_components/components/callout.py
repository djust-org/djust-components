...
class Callout(Component):
    ...
    def _render_custom(self) -> str:
        ...
        icon_html = f'<span class="dj-callout__icon">{html.escape(self.icon)}</span>' if self.icon else ""
        title_html = f'<div class="dj-callout__title">{html.escape(self.title)}</div>' if self.title else ""
        return (
            f'<div class="{cls}">'
            f'{icon_html}'
