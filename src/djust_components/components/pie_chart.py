...
class PieChart(Component):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            segments=segments, title=title, width=width, height=height,
            donut=donut, inner_radius=inner_radius, show_labels=show_labels,
            show_legend=show_legend, custom_class=custom_class, **kwargs,
        )
    ...
    def _render_custom(self) -> str:
        ...
        total = sum(
            float(s.get("value", 0)) for s in self.segments
        ...
        if self.show_legend:
            ...
            for si, seg in enumerate(self.segments):
                ...
                label = html.escape(str(seg.get("label", "")))
                parts.append(
                    f'<rect x="{lx}" y="{ly - 6}" width="10" height="10" '
                    f'rx="2" fill="{color}"/>'
                )
                parts.append(
                    f'<text x="{lx + 14}" y="{ly + 3}" font-size="10">{label}</text>'
                )
                lx += len(label) * 7 + 24
