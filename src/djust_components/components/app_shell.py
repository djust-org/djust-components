...
class AppShell(Component):
    ...
    def _render_custom(self) -> str:
        ...
        sidebar_html = f'<aside class="dj-app-shell__sidebar">{self.sidebar}</aside>' if self.sidebar else ""
        header_html = f'<header class="dj-app-shell__header">{self.header}</header>' if self.header else ""
        return (
            f'<div class="{cls}">'
            f'{sidebar_html}'
