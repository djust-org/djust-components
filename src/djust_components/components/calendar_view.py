...
class CalendarView(Component):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            events=events, month=month, year=year, view=view,
            start_day=start_day, event=event, custom_class=custom_class,
            **kwargs,
        )
    ...
    def _render_custom(self) -> str:
        ...
        dn_cells = "".join(
            f'<div class="dj-calendar__dayname">{html.escape(d)}</div>'
            for d in day_names
        )
        ...
        for week in weeks:
            ...
            for day in week:
                ...
                if len(day_events) > 3:
                    ev_html += (
                        f'<div class="dj-calendar__more">+{len(day_events) - 3} more</div>'
                    )
