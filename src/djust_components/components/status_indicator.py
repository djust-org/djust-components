...
class StatusIndicator(Component):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            status=status, label=label, pulse=pulse,
            size=size, custom_class=custom_class, **kwargs,
        )
        self.status = status
        self.label = label
        self.pulse = pulse
    ...
    def _render_custom(self) -> str:
        ...
        label_html = ""
        if self.label:
            label_html = f'<span class="dj-status-indicator__label">{html.escape(self.label)}</span>'
