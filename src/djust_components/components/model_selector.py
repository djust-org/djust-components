...
class ModelSelector(Component):
    ...
    def _option_inner(opt):
        ...
        ctx_win = html.escape(str(opt.get("context_window", ""))) if opt.get("context_window") else ""
        tier = str(opt.get("tier", "")).lower()
        tier_label = html.escape(ModelSelector.TIER_LABELS.get(tier, tier.capitalize())) if tier else ""
