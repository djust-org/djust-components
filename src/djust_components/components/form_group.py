...
class FormGroup(Component):
    ...
    def _render_custom(self) -> str:
        ...
        label_html = f'<label class="form-label"{for_attr}>{html.escape(self.label)}{required_html}</label>' if self.label else ""
        error_html = f'<span class="form-error-message">{html.escape(self.error)}</span>' if self.error else ""
        helper_html = f'<span class="form-helper">{html.escape(self.helper)}</span>' if self.helper else ""
        return (
            f'<div class="{cls}">'
            f'{label_html}'
            f'{self.content}'
