...
class PageHeader(Component):
    ...
    def _render_custom(self) -> str:
        ...
        subtitle_html = f'<p class="dj-page-header__subtitle">{e_subtitle}</p>' if self.subtitle else ""
        desc_html = f'<p class="dj-page-header__description">{e_description}</p>' if self.description else ""
        actions_html = f'<div class="dj-page-header__actions">{self.actions}</div>' if self.actions else ""
        return (
            f'<header class="{cls}">'
