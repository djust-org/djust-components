...
class PromptEditor(Component):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            template=template, variables=variables, event=event,
            placeholder=placeholder, rows=rows, custom_class=custom_class,
            **kwargs,
        )
    ...
    def _render_custom(self) -> str:
        ...
        for v in unique_vars:
            ...
            preview_text = preview_text.replace(
                "{{" + v + "}}", f'<mark class="dj-prompt-editor__highlight">{html.escape(str(val))}</mark>'
            )
