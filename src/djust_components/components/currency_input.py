...
class CurrencyInput(Component):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            name=name, currency=currency, value=value, label=label,
            min=min, max=max, step=step, placeholder=placeholder,
            event=event, disabled=disabled, required=required,
            error=error, custom_class=custom_class, **kwargs,
        )
        self.name = name
    ...
    def _render_custom(self) -> str:
        ...
        error_html = (
            f'<span class="form-error-message" role="alert">'
            f'{html_mod.escape(self.error)}</span>'
            if self.error else ""
        )
