...
class OrgChart(Component):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            nodes=nodes, root=root, event=event, direction=direction,
    ...
    def _render_node(self, nid, node_map, children, e_event, depth=0):
        ...
        if avatar:
            avatar_html = (
                f'<img class="dj-org__avatar" src="{html.escape(str(avatar))}" '
                f'alt="{name}" />'
            )
        else:
        ...
        child_items = "".join(
            self._render_node(cid, node_map, children, e_event, depth + 1)
