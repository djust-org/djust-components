...
class EmptyState(Component):
    ...
    def _render_custom(self) -> str:
        ...
        icon_html = f'<div class="empty-state-icon">{html.escape(self.icon)}</div>' if self.icon else ""
        title_html = f'<h3 class="empty-state-title">{html.escape(self.title)}</h3>' if self.title else ""
        desc_html = f'<p class="empty-state-description">{html.escape(self.description)}</p>' if self.description else ""
        action_html = ""
