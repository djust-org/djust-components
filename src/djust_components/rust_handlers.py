...
class WizardHandler:
    def render(self, args, content, context):
        ...
        return mark_safe(f'<div class="{class_str}">{nav}<div class="dj-wizard__body">{content}</div></div>')
...
class PageAlertHandler:
    def render(self, args, content, context):
        ...
        icon_html = f'<span class="dj-page-alert__icon">{conditional_escape(str(icon))}</span>' if icon else ""
...
class MeterHandler:
    def render(self, args, context):
        ...
        label_html = f'<div class="dj-meter__label">{conditional_escape(str(label))}</div>' if label else ""
...
class AuditLogHandler:
    def render(self, args, context):
        ...
        col_labels = {"timestamp": "Timestamp", "user": "User", "action": "Action",
                      "resource": "Resource", "detail": "Detail"}

        headers = [f'<th class="dj-audit-log__th">{conditional_escape(col_labels.get(c, c.title()))}</th>'
                   for c in columns]
    ...
    def render(self, args, content, context):
        ...
        if not content or not content.strip():
            ...
            if retry_event:
                ...
                retry_html = f'<button class="dj-error-boundary__retry" dj-click="{e_retry}">Retry</button>'
            return mark_safe(
...
class LineChartHandler:
    ...
    def render(self, args, context):
        ...
        for si, s in enumerate(series):
            ...
            if area and points:
                area_path = (path + f" L{points[-1][0]:.1f},{pad_top + chart_h:.1f}"
        ...
        if show_legend and series:
            ...
            for si, s in enumerate(series):
                ...
                color = conditional_escape(str(s.get("color", self.COLORS[si % len(self.COLORS)])))
                name = conditional_escape(str(s.get("name", f"Series {si + 1}")))
                parts.append(f'<rect x="{lx}" y="{ly - 6}" width="10" height="10" rx="2" fill="{color}"/>')
    ...
    def render(self, args, context):
        ...
        if variant == "bar":
            ...
            if variant == "area" and points:
                area_path = (path + f" L{points[-1][0]:.1f},{pad + chart_h:.1f}"
...
class LogViewerHandler:
    ...
    LEVEL_RE = _re_mod.compile(r'\b(INFO|WARN(?:ING)?|ERROR|DEBUG|TRACE|FATAL|CRITICAL)\b', _re_mod.IGNORECASE)
