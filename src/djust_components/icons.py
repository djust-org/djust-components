"""
Icon system for djust-components.

Provides a shared icon rendering primitive with bundled Heroicons Outline
and support for custom icon sets via DJUST_COMPONENTS_ICON_SETS setting.

Usage in templates::

    {% load djust_components %}
    {% icon name="check" size="md" %}
    {% icon name="x-mark" size="lg" class="text-red" %}

Usage in Python (Rust handlers, component classes)::
...
# ---------------------------------------------------------------------------
# Bundled icon set: Heroicons Outline (24x24 viewBox, stroke-based)
# https://heroicons.com — MIT License
# Stored as SVG inner content (everything inside the <svg> tag).
# ---------------------------------------------------------------------------
HEROICONS_OUTLINE = {
    # Navigation / UI
    ...
    "arrow-path": '<path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182"/>',
    ...
    "computer-desktop": '<path stroke-linecap="round" stroke-linejoin="round" d="M9 17.25v1.007a3 3 0 01-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0115 18.257V17.25m6-12V15a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 15V5.25A2.25 2.25 0 015.25 3h13.5A2.25 2.25 0 0121 5.25z"/>',
    ...
    "link": '<path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.786-3.022a4.5 4.5 0 00-1.242-7.244l-4.5-4.5a4.5 4.5 0 00-6.364 6.364l1.757 1.757"/>',
    ...
    "photo": '<path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M2.25 18V6a2.25 2.25 0 012.25-2.25h15A2.25 2.25 0 0121.75 6v12A2.25 2.25 0 0119.5 20.25H4.5A2.25 2.25 0 012.25 18z"/>',
    ...
    "command-line": '<path stroke-linecap="round" stroke-linejoin="round" d="M6.75 7.5l3 2.25-3 2.25m4.5 0h3m-9 8.25h13.5A2.25 2.25 0 0021 18V6a2.25 2.25 0 00-2.25-2.25H5.25A2.25 2.25 0 003 6v12a2.25 2.25 0 002.25 2.25z"/>',
    ...
    "sparkles": '<path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z"/>',
}
...
def _get_icon_sets():
    ...
    if _icon_sets_cache is not None:
        return _icon_sets_cache

    merged = dict(_BUILTIN_ICON_SETS)
    try:
...
def render_icon(name, size="md", icon_set="heroicons", custom_class="", **attrs):
    ...
    Args:
        name: Icon name (e.g. "check", "x-mark", "sun")
        size: Size preset — "xs" (12px), "sm" (16px), "md" (20px), "lg" (24px),
              or an integer for custom pixel size
        icon_set: Icon set name (default "heroicons")
        custom_class: Additional CSS class(es)
        **attrs: Extra HTML attributes (e.g. aria_label="Close")

    Returns:
        A ``mark_safe()`` HTML string, or empty string if icon not found.
    """
    sets = _get_icon_sets()
    icon_data = sets.get(icon_set, {})
    svg_inner = icon_data.get(name)
    if svg_inner is None:
        return ""

    # Resolve size
    if isinstance(size, str):
        px = ICON_SIZES.get(size, ICON_SIZES["md"])
    else:
        px = int(size)

    # Build CSS classes
    classes = ["dj-icon"]
    size_label = size if isinstance(size, str) and size in ICON_SIZES else None
    if size_label:
        classes.append(f"dj-icon--{size_label}")
    if custom_class:
        classes.append(_html.escape(custom_class))
    class_str = " ".join(classes)

    # Build extra attributes string
    extra = ""
    for key, val in attrs.items():
        # Convert python_name to html-name (aria_label -> aria-label)
        attr_name = _html.escape(key.replace("_", "-"))
